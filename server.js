...
http.listen(3000, function () {
	...
	mongoClient.connect("mongodb+srv://dbb:googlegae@clust.swnog.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", { useUnifiedTopology: true }, function (error, client) {
		if (error) {
			console.log(error);
			return;
		}
				...
				if (user == null) {
					bcrypt.hash(password, 10, function (error3, hash) {
						database.collection("users").insertOne({
							...
							result.render("register", {
								"error": "Email verification is in developmment",
